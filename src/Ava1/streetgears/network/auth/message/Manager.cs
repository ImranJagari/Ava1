...
namespace Ava1.streetgears.network.auth.message
{
    public class Manager
    {
        ...
        {
            var methods = asm.GetTypes().SelectMany(t => t.GetMethods()).Where(m => m.GetCustomAttributes(typeof(AuthAttribute), false).Length > 0).ToArray();
            foreach (var method in methods)
            {
                MethodHandlers.Add((ushort)method.CustomAttributes.ToArray()[0].ConstructorArguments[0].Value, method);
            }
            Log.WriteLog(Log.Type.Info, "Loaded '{0}' auth methods", methods.Length);
        }
