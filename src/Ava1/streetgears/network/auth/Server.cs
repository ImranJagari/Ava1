...
namespace Ava1.streetgears.network.auth
{
    public class Server
    {
        ...
        {
            _server = new BaseServer(Config.elements["Auth"]["Ip"], int.Parse(Config.elements["Auth"]["Port"]));
        ...
        {
            Log.WriteLog(Log.Type.Info, "Started auth server on port '{0}'", Config.elements["Auth"]["Port"]);
        }
        ...
        private static void server_OnServerAcceptedSocket(BaseClient socket)
        {
            ...
            //Log.WriteLog(Log.Type.Info, "'{0}:{1}' connected to Auth server", socket.ip, socket.prt);
        }
