...
namespace Ava1.streetgears.network.relay
{
    public class Server
    {
        ...
        {
            _server = new BaseServer(Config.elements["Relay"]["Ip"], Int16.Parse(Config.elements["Relay"]["Port"]));
        ...
        {
            Log.WriteLog(Log.Type.Info, "Started relay server on port '{0}'", Config.elements["Relay"]["Port"]);
        }
        ...
        private static void server_OnServerAcceptedSocket(BaseClient socket)
        {
            ...
            //Log.WriteLog(Log.Type.Info, "'[{0}:{1}]' connected to relay server", socket.ip, socket.prt);
        }
