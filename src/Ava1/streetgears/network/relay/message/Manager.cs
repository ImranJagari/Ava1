...
namespace Ava1.streetgears.network.relay.message
{
    public class Manager
    {
        ...
        {
            var methods = asm.GetTypes().SelectMany(t => t.GetMethods()).Where(m => m.GetCustomAttributes(typeof(RelayAttribute), false).Length > 0).ToArray();
            foreach (var method in methods)
            {
                MethodHandlers.Add((ushort)method.CustomAttributes.ToArray()[0].ConstructorArguments[0].Value, method);
            }
            Log.WriteLog(Log.Type.Info, "Loaded '{0}' relay methods", methods.Length);
        }
